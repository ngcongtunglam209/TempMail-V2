...
| Domain                     | Role                                                   |
| -------------------------- | ------------------------------------------------------ |
| `yunaalways.cyou`          | Webmail UI users visit                                 |
...
- Frontend at `https://yunaalways.cyou` (and `https://www.yunaalways.cyou`)
- API at `https://api.yunaalways.cyou`
...
> short domain you own for the webmail side; this guide uses `yunaalways.cyou` as the
> example.
...
### Webmail domain — `yunaalways.cyou`

| Name           | Type | Value     |
| -------------- | ---- | --------- |
| `yunaalways.cyou` (apex) | A   | SERVER_IP |
| `www`          | A    | SERVER_IP |
| `api`          | A    | SERVER_IP |
...
The repo ships configured for `yunaalways.cyou` (webmail) and `vietkieu.edu.pl` (mail).
If your webmail domain is something else, edit two files **before** running
the installer:

1. `deploy/Caddyfile` — replace `yunaalways.cyou` and `api.yunaalways.cyou` with your real
   webmail hostnames.
2. `deploy/install.sh` — change `CORS_ORIGINS=https://yunaalways.cyou,...` to match.
...
```bash
# API is up
curl -s https://api.yunaalways.cyou/v1/health | jq

# Webmail is up
curl -sI https://yunaalways.cyou/ | head -n1
...
you generated through the UI on `yunaalways.cyou`). It should appear in the inbox
...
`dig yunaalways.cyou` and `dig api.yunaalways.cyou` to confirm A records resolve to your
...
origin (`https://yunaalways.cyou`). Edit and `sudo systemctl restart lammail`.
